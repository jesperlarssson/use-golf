...
// SVG i stället för tecknet ↗, som iOS annars visar som emoji.
function ArrowUpRight({ className = "size-3" }: { className?: string }) {
  return <svg viewBox="0 0 12 12" fill="none" aria-hidden="true" className={className}><path d="M3 9 9 3M4 3h5v5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="square" /></svg>;
}
  ...
  return <footer className="pb-24 lg:pb-0">
    <section aria-labelledby="instagram-heading" className="border-t border-black/15 px-6 py-16 sm:px-10 lg:px-16 lg:py-20">
      <div className="mx-auto max-w-screen-2xl">
        <div data-reveal className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><h2 id="instagram-heading" className="text-balance text-3xl font-normal tracking-[-.035em] sm:text-4xl">Följ livet på USE.</h2></div><a href={instagram} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center gap-2 text-sm underline underline-offset-8">Följ @use__golf på Instagram <ArrowUpRight /></a></div>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{photos.map(photo => <a key={photo.src} href={instagram} target="_blank" rel="noopener noreferrer" aria-label={`${photo.alt} – besök USE på Instagram`} data-reveal="image" className="group relative aspect-square overflow-hidden outline outline-1 -outline-offset-1 outline-black/10"><Image src={photo.src} alt={photo.alt} fill sizes="(max-width: 767px) 50vw, 25vw" className="object-cover transition-transform duration-500 motion-safe:group-hover:scale-[1.03]" /><span aria-hidden="true" className="absolute right-3 bottom-3 flex size-8 items-center justify-center bg-black/40 text-white"><ArrowUpRight className="size-3.5" /></span></a>)}</div>
      </div>
    </section>
    <div className="bg-[#293329] text-[var(--brand-primary)]">
      <div className="mx-auto grid max-w-screen-2xl gap-10 px-6 py-16 sm:grid-cols-2 sm:px-10 lg:grid-cols-[1.5fr_1fr_1fr] lg:px-16">
        ...
        <div><h3 className="mb-5 text-xs uppercase tracking-[.18em] text-white/60">Säg hej</h3><a href="mailto:hello@usegolf.se" className="flex min-h-11 items-center text-sm hover:underline">hello@usegolf.se</a><a href="tel:+46767174034" className="flex min-h-11 items-center text-sm hover:underline">076-717 40 34</a><a href={instagram} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center gap-1.5 text-sm hover:underline">Instagram <ArrowUpRight /></a></div>
      </div>
