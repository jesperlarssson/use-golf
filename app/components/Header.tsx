  ...
  return <>
    <header className={`fixed inset-x-0 top-0 z-50 border-b transition-colors duration-200 ${light ? "border-white/20 bg-black/10 text-[var(--brand-primary)]" : "border-black/10 bg-[var(--brand-primary)] text-[var(--foreground)]"}`}>
      <div className="mx-auto flex h-20 max-w-screen-2xl items-center justify-between gap-3 px-4 sm:gap-6 sm:px-10 lg:px-16">
        ...
        <nav aria-label="Huvudmeny" className="ml-auto hidden lg:block"><ul className="flex items-center gap-7">{navigation.map(item => <li key={item.href}><Link href={item.href} aria-current={pathname === item.href ? "page" : undefined} className={`inline-flex min-h-11 items-center text-sm transition-opacity hover:opacity-65 ${pathname === item.href ? "underline underline-offset-8" : ""}`}>{item.label}</Link></li>)}</ul></nav>
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <Link href="/bokning" className={`inline-flex min-h-11 shrink-0 items-center justify-center gap-6 whitespace-nowrap px-4 sm:px-5 text-sm font-medium transition-colors active:scale-[.96] ${light ? "bg-[var(--brand-primary)] text-[var(--foreground)] hover:bg-white" : "bg-[var(--brand-olive-900)] text-white hover:bg-[#3e4837]"}`}>Boka nu <span aria-hidden="true" className="hidden sm:inline">↗</span></Link>
    ...
    <dialog ref={dialog} id="mobile-menu" aria-label="Huvudmeny" onCancel={() => setMenuOpen(false)} onClose={() => setMenuOpen(false)} className="fixed inset-0 m-0 h-[100dvh] max-h-none w-full max-w-none bg-[var(--brand-primary)] p-6 text-[var(--foreground)] backdrop:bg-black/40" onClick={e => { if (e.target === e.currentTarget) setMenuOpen(false); }}>
      ...
      <nav aria-label="Mobilmeny" className="mt-12"><ul>{navigation.map(item => <li key={item.href} className="border-b border-black/15"><Link href={item.href} onClick={() => setMenuOpen(false)} aria-current={pathname === item.href ? "page" : undefined} className="block py-5 text-3xl tracking-tight">{item.label}</Link></li>)}</ul><Link href="/bokning" onClick={() => setMenuOpen(false)} className="mt-8 flex min-h-14 items-center justify-between bg-[var(--brand-olive-900)] px-6 text-white">Boka nu <span aria-hidden="true">↗</span></Link><Link href="/kontakt" onClick={() => setMenuOpen(false)} className="mt-6 inline-flex min-h-11 items-center underline underline-offset-4">Kontakt & hitta hit</Link></nav>
    </dialog>
    <div className="mobile-booking fixed inset-x-0 bottom-0 z-40 border-t border-black/10 bg-[var(--brand-primary)] px-4 pt-3 pb-[max(.75rem,env(safe-area-inset-bottom))] lg:hidden"><Link href="/bokning" className="flex min-h-12 items-center justify-center gap-8 bg-[var(--brand-olive-900)] text-sm font-medium text-white">Boka nu <span aria-hidden="true">↗</span></Link></div>
  </>;
}
