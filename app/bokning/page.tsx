...
export default function BookingPage() {
  return <>
    ...
    <section className="mx-auto grid max-w-screen-2xl gap-10 px-6 py-20 sm:px-10 md:grid-cols-[.8fr_1.2fr] lg:px-16 lg:py-28">
      <div data-reveal><h2 className="max-w-md text-balance text-4xl font-normal leading-[1.1] tracking-[-.035em] md:text-5xl">Enkelt att komma i gång.</h2><p className="mt-6 max-w-sm text-base leading-relaxed text-[var(--foreground)]/75">Du behöver inte vara medlem. Välj en tid som passar, så ses vi i Nya Hovås.</p></div>
      ...
      ].map(item => <li key={item.title} data-reveal className="border-b border-black/20 py-7"><div><h3 className="text-2xl font-normal tracking-tight">{item.title}</h3><p className="mt-3 max-w-lg text-sm leading-relaxed text-[var(--foreground)]/75">{item.text}</p></div></li>)}</ol>
    </section>
