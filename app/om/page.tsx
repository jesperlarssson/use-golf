...
export default function OmPage() {
  return <>
    <EditorialHero label="Om USE Golf" title="Golf på dina villkor." description="Vi startade USE för att göra golfen mer tillgänglig, mer social och roligare. En plats att spela året runt – och gärna stanna en stund till." image="/images/people/7.png" imageAlt="Golf och gemenskap hos USE Golf">
      <Link href="/bokning" className="inline-flex min-h-14 items-center justify-center gap-10 bg-[var(--brand-primary)] px-7 text-sm text-[var(--foreground)] transition-colors hover:bg-white">Boka simulator <span aria-hidden="true">↗</span></Link>
    </EditorialHero>
    ...
    <section className="bg-[var(--brand-olive-900)] text-[var(--brand-primary)]"><div className="mx-auto flex max-w-screen-2xl flex-col justify-between gap-8 px-6 py-16 sm:px-10 md:flex-row md:items-center lg:px-16 lg:py-20"><div><p className="mb-5 text-xs uppercase tracking-[.2em] text-white/75">Välkommen till Nya Hovås</p><h2 className="text-3xl font-normal tracking-[-.035em] md:text-4xl">Gör nästa runda till en kväll.</h2></div><Link href="/kontakt" className="inline-flex min-h-14 items-center justify-center gap-10 border border-white/50 px-7 text-sm transition-colors hover:bg-white/10">Hitta till USE <span aria-hidden="true">↗</span></Link></div></section>
  </>;
}
