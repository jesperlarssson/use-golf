  ...
  return <>
    <section className="relative isolate flex min-h-[640px] items-end bg-[var(--brand-olive-900)] text-[var(--brand-primary)]">
      ...
      <div className="relative mx-auto w-full max-w-screen-2xl px-6 pb-16 pt-40 sm:px-10 lg:px-16 lg:pb-20">
        ...
        <a href="#forfragan" className="cta-sweep mt-8 inline-flex min-h-14 items-center gap-10 bg-[var(--brand-primary)] px-7 text-sm text-[var(--foreground)] transition-colors">Planera ert event</a>
      </div>
    </section>
    <section className="mx-auto max-w-screen-2xl px-6 py-20 sm:px-10 lg:px-16 lg:py-28">
      <h2 className="mb-12 text-balance text-4xl font-normal tracking-[-.035em] md:text-5xl">Samla kollegor och kunder på USE.</h2>
      <div className="grid gap-10 md:grid-cols-3">{[
        ...
        <h3 className="text-2xl font-normal tracking-tight">{item.title}</h3><p className="mt-4 text-sm leading-relaxed text-[var(--foreground)]/75">{item.text}</p><a href="#forfragan" className="mt-5 inline-flex min-h-11 items-center gap-6 text-sm underline underline-offset-8">Planera ert event</a>
      </article>)}</div>
    </section>
