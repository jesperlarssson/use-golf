    ...
    <section aria-label="Hos USE Golf" className="bg-[#293329] text-[var(--brand-primary)]">
      <ul className="mx-auto grid max-w-screen-2xl grid-cols-1 px-6 sm:grid-cols-2 py-5 sm:px-10 lg:grid-cols-4 lg:px-16 lg:py-10">
        {highlights.map((item, index) => (
          <li key={item.title} data-reveal className={`flex items-center gap-3 py-7 text-left ${index > 0 ? "border-t border-white/20" : ""} ${index % 2 === 0 ? "sm:pr-4" : "sm:border-l sm:border-white/20 sm:pl-4"} ${index === 1 ? "sm:border-t-0" : ""} lg:border-t-0 lg:px-6 lg:first:pl-0 lg:py-2 lg:[&:not(:first-child)]:border-l`}>
